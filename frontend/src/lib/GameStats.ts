...
type CharacterStatsType = Record<StatisticType, number>;

type CharacterStateType = {
  isWarrior: boolean;
};

type GameStateType = Record<Character, CharacterStateType>;

export type ChangeFilter = Partial<Record<Character, StatisticType[]>>;

export type GameStatsType = Record<Character, CharacterStatsType>;
...
export class GameStats extends ProtectedEventEmitter<Events> {
  ...
  private static get statisticTypes(): StatisticType[] {
    return ["kills", "queen_kills", "warrior_kills", "deaths"];
  }
  /**
  ...
  private static isQueen(character: Character): boolean {
    return (
      character === Character.GoldQueen || character === Character.BlueQueen
    );
  }
  ...
  trigger(eventType: "change", filter?: ChangeFilter) {
    ...
    for (let [character, statistics] of Object.entries(filter)) {
      ...
      if (!isNaN(characterNumber)) {
        for (let statistic of statistics) {
          this.protectedEmit("change", {
            ...
            statistic: statistic,
            value: this.gameStats[characterNumber as Character][statistic],
          });
        }
      }
    }
  ...
  private processKill(kill: PlayerKill) {
    ...
    if (
      kill.killed === Character.GoldQueen ||
      kill.killed === Character.BlueQueen
    ) {
